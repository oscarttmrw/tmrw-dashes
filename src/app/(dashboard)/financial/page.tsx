...
export default function FinancialPage() {
  ...
  // "Today" anchors MTD / projection. Use the latest financial_revenue date,
  // falling back to the latest stripe invoice, then the system clock.
  const today = useMemo(() => {
    let maxTs = 0
  ...
  const projectedMonthEnd = dayOfMonth > 0 ? (mtd.net / dayOfMonth) * dim : 0
  ...
  const buildOverlay = useCallback((rows: Row[]) => {
    ...
    const keys = Array.from(grouped.keys()).sort().slice(-6)
    return keys.map(k => {
